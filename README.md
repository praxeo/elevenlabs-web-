...
2. **Phone-link delivery durability.** A note dictated while the desktop link was down can no longer be stranded: queued deliveries are code-bound and **survive Leave/rejoin** (re-linking with the same code delivers them immediately), a **"⏳ N waiting" chip** on both layouts makes the queue visible (tap to retry), and **"📤 Send to desktop"** (on the box, history rows, and the phone's expanded transcript peek) re-sends any past note on demand. *Next:* room-side persistence + a desktop poll fallback so a dead WebSocket can't delay delivery either.
3. **Settings portability.** Sync the portable settings across devices while keeping per-device tuning local — export/import or passphrase-keyed server-side profiles.
4. **Phone UI.** The big-button layout is done; surfacing the capture waveform on the big screen is the open gap (the Mic ✓/⚠ health pill landed), then a larger live readout and swipe gestures.
