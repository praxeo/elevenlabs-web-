...
6. **Batch latency, mic accuracy, and the minimized-window recording cue.** Planned in **`PERF_PLAN.md`**: instrument the release→clipboard budget first (per-stage client timings + a `Server-Timing` split of transport vs. ElevenLabs inference), then the certain wins — a whole-window recording wash for a shrunk PWA on a machine with no speakers, an AutoHotkey focus-return so a dictation no longer ends in an alt-tab, and a gate look-ahead + release tail to stop clipping the first and last words — then the mic levers (device picker, narrowband warning, gate-bypass A/B). PCM upload and transport streaming are conditional on what the measurements say.
