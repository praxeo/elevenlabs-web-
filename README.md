...
3. **Phone-link security — access-code gate landed.** The session routes (`/deliver`, `/latest`, `/status`, the listener WebSocket) now require the shared access code whenever the worker has one configured, with per-IP rate caps (guess-scanning and DO-cost abuse) and a delivery size cap; a missing/wrong code is a loud, named failure on both surfaces, and the refused note stays queued. *Next:* end-to-end encryption of deliveries (key in the QR fragment, ciphertext-only relay) so patient text never rests readable on the room, plus a WAF rate rule in front.
